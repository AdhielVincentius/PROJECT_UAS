            ...
            <div class="wrap-login100">
                <form method="post" action="" class="login100-form validate-form">
                    <span class="login100-form-title p-b-48">
                        ...
                        <img src="./assets/img/USD.png" width="200">
                    </span>
				...
				if ($_SERVER['REQUEST_METHOD']=='POST') {
					...
					if ($level==1) {
						// Dosen
						$sqlCek = mysqli_query($con,"SELECT * FROM tb_dosen WHERE nip='$_POST[username]' AND password='$pass' AND status='Y'");
						...
						// Rektorat
								$sqlCek = mysqli_query($con,"SELECT * FROM tb_rektorat WHERE nip='$_POST[username]' AND password='$pass' AND status='Y'");
								$jml = mysqli_num_rows($sqlCek);
								$d = mysqli_fetch_array($sqlCek);
								
								if ($jml > 0) {
								$_SESSION['rektorat']= $d['id_rektor'];
								...
								swal('($d[nama_rektor]) ', 'Login berhasil', {
								icon : 'success',
								buttons: {        			
								confirm: {
								className : 'btn btn-success'
								...
								window.setTimeout(function(){ 
								window.location.replace('./rektor/');
								} ,3000);   
								</script>";
